...
*   **ルール**:
    ...
    - **State Transparency（状態の透明性）**: Conceptの状態（State）は外部（Sync層）から「読み取り可能」とし、ゲッター関数の連鎖を排除せよ。書き込みはActionのみが行う（読み取り全開放、書き込み厳封）。
...
*   **ルール**:
    ...
    - **Action Traces（因果の署名）**: 全てのアクション記録に「どのSyncルールによって実行されたか」の署名を残せ。デバッグ時に因果のグラフを遡るだけで原因を特定可能にする。
