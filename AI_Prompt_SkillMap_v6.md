...
*   **コンテキストエンジニアリング（Context Engineering）**
    *   Andrej Karpathyが提唱する「適切な情報を適切なタイミングで供給する科学と芸術」。
    *   **4つの戦略**:
        *   **Write（書き込み）**: 思考の下書きや長期記憶もたせる。
        *   **Select（選別）**: RAG等で必要な情報だけを選び出す。
        *   **Compress（圧縮）**: 膨大なログを要約し、核心を残す。
        *   **Isolate（隔離）**: 複雑なタスクをマルチエージェントに分割する。
*   **システムプロンプト設計**（エージェントの常駐設定・人格設計）
*   外部データ連携（API・Vector DB）とパイプライン構築
